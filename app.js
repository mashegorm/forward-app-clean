...
function enter(p, loading=true){
 ...
 setTimeout(()=>{$("#loadingScreen").classList.add("hidden");$("#app").classList.remove("hidden");setProfile(p);renderAll();}, loading?450:0);
}
...
function renderAll(){renderStories();renderFeed();renderRooms();renderMarket();applyAdmin();}
...
function renderMarket(){
 $("#marketList").innerHTML=offers.map(o=>`<div class="market-card"><div class="product-image">${o.title.split(" ")[0]}</div><h3>${o.title}</h3><p>${o.price}</p><button>${o.button}</button>${admin?'<div class="admin-tools"><button>Edit Offer</button><button class="danger">Delete Offer</button></div>':""}</div>`).join("");
}
...
$("#adminToggle").onclick=()=>{admin=!admin;$("#adminToggle").textContent=admin?"Admin On":"Admin";renderFeed();renderMarket();applyAdmin(); if($("#rooms").classList.contains("active"))renderRooms();}
