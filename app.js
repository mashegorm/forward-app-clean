...
function setProfile(profile){
  ...
  $("#memberMeta").textContent = `Level 1 • ${profile.contact || "Access Member"}`;
  $("#memberProvince").textContent = profile.province || "-";
  $("#memberIndustry").textContent = profile.industry || "-";
  $("#memberAge").textContent = profile.age || "-";
...
$("#adminSwitch").addEventListener("click", ()=>{
  ...
  if($("#roomView").classList.contains("active")) openRoom();
});
...
function renderRooms(){
  ...
  list.innerHTML = rooms.map((r,i)=>`
    <button class="room-card" data-room="${i}">
      ...
      <div class="room-meta">
        <span class="status ${r.locked || r.readOnly ? "closed" : ""}">${r.readOnly ? "Read-only" : (r.locked ? "Closed" : "Open")}</span>
        <span class="count">${r.members} Members</span>
      </div>
    </button>
  `).join("");
...
function openRoom(){
  ...
  $("#roomState").textContent = r.readOnly ? "Read-only room • reactions enabled" : (r.locked ? "Closed room • reactions enabled" : "Open room");
  $("#messageComposer").classList.toggle("hidden", (r.readOnly || r.locked) && !adminMode);
  showPage("roomView");
  renderMessages();
}
