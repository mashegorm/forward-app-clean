...
function fileToData(file){return new Promise((res,rej)=>{if(!file)return res(null);let r=new FileReader();r.onload=()=>res({name:file.name,type:file.type,url:r.result});r.onerror=rej;r.readAsDataURL(file)})}
...
showReelCreate.onclick=()=>reelCreate.classList.toggle("hidden");createReel.onclick=async()=>{let f=await fileToData(reelFile.files[0]);if(!f)return alert("Choose video/image first.");state.reels.unshift({title:reelTitle.value||"New Reel",caption:reelCaption.value||"",media:f});reelTitle.value=reelCaption.value=reelFile.value="";save();renderReels()}
function renderReels(){reelStack.innerHTML=state.reels.map((r,i)=>`<div class="reel">${mediaHTML(r.media)}<div><h2>${r.title}</h2><p>${r.caption||""}</p><div>❤️ 👍 😊 ✅ 💯</div>${state.admin?`<button class="read-more" onclick="deleteReel(${i})">Delete</button>`:""}</div></div>`).join("")}
window.deleteReel=i=>{state.reels.splice(i,1);save();renderReels()}
showEventCreate.onclick=()=>eventCreate.classList.toggle("hidden");createEvent.onclick=async()=>{state.events.unshift({title:eventTitleInput.value||"New Event",date:eventDateInput.value||"Coming Soon",price:eventPriceInput.value||"Free",details:eventDescInput.value||"",media:await fileToData(eventPoster.files[0])});eventTitleInput.value=eventDateInput.value=eventPriceInput.value=eventDescInput.value=eventPoster.value="";save();renderEvents()}
function renderEvents(){eventsList.innerHTML=state.events.map((e,i)=>`<div class="event-card"><div class="event-poster" ${e.media?`style="background-image:url('${e.media.url}')"`:""}>${e.media?"":e.title}</div><h3>${e.title}</h3><p>${e.date} • ${e.price}</p><button class="read-more" onclick="toggleDetails('event${i}')">See More Details</button><div class="details" id="event${i}"><p>${e.details}</p><button>Register Interest</button>${state.admin?`<button class="danger" onclick="deleteEvent(${i})">Delete</button>`:""}</div></div>`).join("")}
...
function assignRole(role){let c=roleContact.value.toLowerCase();if(!c)return alert("Enter member email/phone.");state.roles[c]=role;if(state.profiles[c])state.profiles[c].role=role;roleContact.value="";save();renderRoles();alert(`${role} assigned`)}
function renderRoles(){roleList.innerHTML=Object.entries(state.roles).map(([c,r])=>`${c} — ${r}`).join("<br>")}
