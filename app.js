...
function fileToData(file){
  ...
  return Promise.resolve({name:file.name,type:file.type,url:URL.createObjectURL(file)});
}
...
function renderReels(){
  reelStack.innerHTML=state.reels.map((r,i)=>{
    let mediaBlock = r.media ? (r.media.type&&r.media.type.startsWith("video") ? `<div class="media"><video src="${r.media.url}" controls playsinline></video></div>` : `<div class="media"><img src="${r.media.url}"></div>`) : "";
    return `<div class="reel">${mediaBlock}<div><h2>${r.title}</h2><p>${r.caption||""}</p><div>❤️ 👍 😊 ✅ 💯</div>${state.admin?`<button class="read-more" onclick="deleteReel(${i})">Delete</button>`:""}</div></div>`;
  }).join("");
}
...
if(state.activeContact&&state.profiles[state.activeContact])enter(false);


/* V35 FINAL CLEAN REEL MODULE */
window.forwardV35ReelURLs = {};
function initV35Reels(){
  if(!state.reels || !Array.isArray(state.reels)){
    state.reels = [];
  }

  const showBtn = document.getElementById("showReelCreate");
  const panel = document.getElementById("reelCreate");
  const file = document.getElementById("reelFile");
  const selected = document.getElementById("reelSelected");
  const post = document.getElementById("createReel");
  ...
  if(file){
    file.onchange = () => {
      ...
      selected.textContent = f ? "Selected: " + f.name : "No reel selected yet.";
    };
  }
  ...
  if(post){
    post.onclick = () => {
      ...
      const key = "v35_" + Date.now();
      const url = URL.createObjectURL(f);
      window.forwardV35ReelURLs[key] = { url, type:f.type, name:f.name };
      ...
      file.value = "";
      selected.textContent = "No reel selected yet.";
...
renderReels = function(){
  ...
  stack.innerHTML = state.reels.map((r,i)=>{
    let media = "";
    let canOpen = false;

    if(r.v35Key && window.forwardV35ReelURLs[r.v35Key]){
      const live = window.forwardV35ReelURLs[r.v35Key];
      canOpen = true;
      media = live.type && live.type.startsWith("video")
        ? `<video src="${live.url}" controls playsinline preload="metadata"></video>`
        : `<img src="${live.url}" alt="">`;
    } else if(r.media && r.media.url){
      canOpen = true;
      media = r.media.type && r.media.type.startsWith("video")
        ? `<video src="${r.media.url}" controls playsinline preload="metadata"></video>`
        : `<img src="${r.media.url}" alt="">`;
    }

    const missing = (r.v35Key && !window.forwardV35ReelURLs[r.v35Key])
      ? `<p class="reel-status">This uploaded reel needs repost after refresh until backend storage is connected.</p>`
      : "";
...
window.openReel = function(i){
  const r = state.reels[i];
  let url = "";
  let type = "";

  if(r && r.v35Key && window.forwardV35ReelURLs[r.v35Key]){
    url = window.forwardV35ReelURLs[r.v35Key].url;
    type = window.forwardV35ReelURLs[r.v35Key].type;
  } else if(r && r.media && r.media.url){
    url = r.media.url;
    type = r.media.type || "";
  }

  if(!url){
    alert("This uploaded reel needs repost after refresh until backend storage is connected.");
    return;
  }
...
window.deleteReel = function(i){
  const r = state.reels[i];
  if(r && r.v35Key && window.forwardV35ReelURLs[r.v35Key]){
    try{ URL.revokeObjectURL(window.forwardV35ReelURLs[r.v35Key].url); }catch(e){}
    delete window.forwardV35ReelURLs[r.v35Key];
  }
