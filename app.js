...
function sortForwardRoomsV9(rooms){
  const order = window.FORWARD_ROOMS.map(r => r.name);
  return rooms.sort((a,b)=>{
    if(a.name === "General Access & Announcements") return -1;
    if(b.name === "General Access & Announcements") return 1;
    return order.indexOf(a.name) - order.indexOf(b.name);
  });
}

async function renderRooms(){
  roomsCache = sortForwardRoomsV9(await db("forward_rooms?select=*&order=created_at.asc"));
  ...
  const visibleRooms = searchTerm ? roomsCache.filter(r => (r.name + " " + (r.description || "")).toLowerCase().includes(searchTerm)) : roomsCache;
...
function renderVault(){
  ...
  const first = (m.name || "Forward").trim();
  const surname = (m.surname || "Member").trim();
  const fullName = `${first} ${surname}`.trim();
  const province = m.province || "Province";
  const industry = m.industry || "Industry";
  const age = m.age_range || "Age Range";
  const role = m.role === "admin" ? "Admin" : "Member";

  if($("#vaultName")) $("#vaultName").textContent = fullName;
  if($("#vaultMeta")) $("#vaultMeta").textContent = `${province} · ${industry} · ${age}`;
  if($("#vaultRole")) $("#vaultRole").textContent = role;
  if($("#vaultAvatar")) $("#vaultAvatar").textContent = (first[0] || "F").toUpperCase();
}
...
// Existing members can still sign in using their ID number.
// Database records are not touched.
if(activeMember){
  $("#signinId").value = activeMember.id_number || "";
  $("#signinTab").click();
}
